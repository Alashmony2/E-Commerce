        ...
        <div className="justify-between items-center mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
            ...
            <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                ...
                <Button isLoading={decrementIsLoading} onPress={() => updateProductCount(product.product._id,product.count - 1,setIncrementIsLoading,setDecrementIsLoading,product.count)}
                    className="cursor-pointer rounded-none bg-gray-100 min-w-0 duration-100 hover:bg-blue-500 hover:text-blue-50">
                        - 
                </Button>
                <input className="w-10 border h-10 bg-white text-center text-xs outline-none" type="number" value={productCount} onChange={(e) => setProductCount(e.target.value)} onBlur={(e) => updateProductCount(product.product._id,e.target.value,setIncrementIsLoading,setDecrementIsLoading,product.count)} min={1} />
